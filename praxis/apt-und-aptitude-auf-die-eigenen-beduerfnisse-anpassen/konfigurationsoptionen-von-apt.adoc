...
ähnlich zu `Aptitude::Clean-After-Install`. Default ist `False`, d.h. es
findet kein Aufräumen statt.

`Aptitude::Auto-Fix-Broken` :: Mit dieser Option fragt Aptitude nach einer
Bestätigung, bevor es versucht, beschädigte Pakete zu reparieren. Default 
ist `True`, d.h. keine Rückfrage vor der Reparatur.
...
Installation oder Aktualisierung markiert haben. Default ist `True`, d.h. 
die Recherche zu den Paketabhängigkeiten ist aktiviert.

`Aptitude::Auto-Install-Remove-Ok` :: Hiermit entfernt Aptitude 
automatisch in Konflikt stehende Pakete, wenn Sie ein Paket zur 
Installation oder Aktualisierung markieren. Normalerweise werden diese 
Konflikte zunächst markiert und sind von Ihnen manuell zubeheben. Default
ist `False`, d.h. Aptitude nimmt keine automatische Entfernung vor.
