...
`debsigs`. Nachfolgender Aufruf signiert das Paket 'meta-mc' mit dem Schlüssel
`D431AC07`. Die Angabe `--sign=origin` ist die kryptographische Signatur der 
Organisation, die das Paket bereitstellt. Erlaubte Alternativen sind `maint` für
den Paketmaintainer und `archive` für das Archiv, welches das Paket bereitstellt.
