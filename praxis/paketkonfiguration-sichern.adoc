...
<<paketkonfiguration-sichern-mit-dpkg>>), `debconf-get-selections`, 
`debconf-set-selections` und `apt-clone` (siehe 
<<paketkonfiguration-sichern-mit-apt-clone>>). Zu nutze machen wir uns dabei 
auch die Beispiele zum Filtern der bereits auf dem System installierten 
Pakete aus dem Abschnitt <<liste-der-installierten-pakete-anzeigen-und-deuten>>. 
...
nachfolgenden Listing sehen Sie, dass das nicht immer gelingt und auch zu
Paketen führen kann, die nicht wiedereinspielbar sind:
