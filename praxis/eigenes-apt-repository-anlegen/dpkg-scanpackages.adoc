...
die Datei `Packages.gz` entsteht. Der Parameter `-c` von `gzip` sorgt 
dafür, dass `gzip` seine Ausgabe auf die Standardausgabe schreibt. 
Abschließend vermeldet `dpkg-scanpackages`, wieviele Pakete es gefunden
hat und in die Ausgabedatei geschrieben hat:
