...
nachfolgende Aufruf schließt alle Dateien namens `copyright` aus, 
die sich in einem Verzeichnis von `/usr/share/doc/` befinden.
...
WARNING: Geben Sie mit diesem Aufruf das falsche Verzeichnis mit,
können Sie damit ihr System schwer schädigen. Installierte Programme
oder Bibliotheken sind unvollständig und funktionieren dann nicht
wie vorgesehen.
