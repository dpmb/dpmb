...
aktuellen Maschine passen. Genutzt wird das Werkzeug im Rahmen der
Hardwareerkennung des Debian-Installers, um diejenigen Pakete zu 
identifizieren, welche beim Setup benötigt werden. Im Gegensatz dazu
wertet `isenkram-autoinstall-firmware` die Ausgaben von `dmesg` aus und 
sucht darin nach Ausgaben zu fehlenden Treibern. Es aktiviert dazu die 
