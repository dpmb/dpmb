...
Spiegelservers 'dak' und 'minidak'. 'dak' <<DebianArchiveKit>> kürzt 'Debian 
Archive Kit' ab, ist die offizielle Lösung und besitzt eine Anbindung an das 
DBMS PostgreSQL. 'minidak' <<minidak>> ist eine Reimplementierung von 'dak' 
ohne Anbindung an eine Datenbank. Die offiziellen Empfehlungen für private
Spiegelserver sind 'reprepro' (siehe <<reprepro>>) und 'minid-install' (siehe
