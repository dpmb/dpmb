...
Ist das erlaubt? Ja, klar -- hier bestehen prinzipiell keinerlei 
Einschränkungen. Niemand hindert Sie daran, Software aus diesen Quellen in 
Form von Debianpaketen einzuspielen, wenn Sie das möchten.

Sollten Sie das tun? Nein, nicht unbedingt. Aber es geht doch. Ja klar, 
trotzdem sollten Sie das nicht einfach so machen! Falls doch, sind ein 
paar Tips hilfreich, die die damit verbundenen Risiken minimieren. Hier 
folgen nun zunächst ein paar Gründe, welche Risiken bei Fremdpaketen 
überhaupt bestehen, die ``einfach so von irgendwoher'' heruntergeladen und
eingespielt werden:
...
* keine vorherige Prüfung auf Paketabhängigkeiten. Das Paket mag auf dem
  ...
  Garantie für das Funktionieren auf Systemen, die reguläre 
  Veröffentlichungen nutzen 
* keine Eintragung in die Liste bereitgestellter Pakete, d.h. zukünftig 
  ist mehr Arbeit für Sie als Systembetreuer notwendig. Es steht 
  einerseits eine manuelle Aktualisierung des Pakets an, sprich: alles 
  das, was ein Maintainer machen würde. Außerdem müssen Sie prüfen, ob 
  alle Abhängigkeiten erfüllt sind, um das Paket weiter benutzen zu können
...
den gleichen Eigenschaften oder Funktionsumfang (siehe 
...
geprüft haben und für unbedenklich befinden, spielen Sie dieses mittels 
`dpkg -i` 'Dateiname' ein (siehe auch <<pakete-installieren>>).
...
(((reportbug)))
(((RFP)))
(((Software für Debian paketieren)))
Was soll ich tun, dass das von mir genutzte Paket endlich in Debian als 
offizielles Paket verfügbar wird? Wir sehen diese Möglichkeiten zur 
Lösung:
...
* kontaktieren Sie einen Paketmaintainer für Debian und fragen Sie ihn, ob
  er Interesse hat, die von ihnen vorgeschlagene Software zu paketieren
