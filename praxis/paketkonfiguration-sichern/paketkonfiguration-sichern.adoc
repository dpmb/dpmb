...
sind hier zwei kleine Hindernisse zu umschiffen. 

Erstens sortiert `apt` intern anders als `dpkg`, zweitens ist das Ausgabeformat 
von `apt` anders -- die Architektur eines Paketes wird in einer zusätzlichen 
Spalte benannt. Daher werden zunächst die erste und vierte Spalte zusammengezogen
und danach die Angabe der Architektur `all` mittels `sed` entfernt.
