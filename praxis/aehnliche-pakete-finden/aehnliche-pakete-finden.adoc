...
* wie identifiziere ich im Paketbestand mögliche Alternativen B, C und D, 
  ...
  läßt sich Werkzeug A möglicherweise ersetzen
...
* jedes Paket hat in den Metadaten einen Eintrag zur Kategorie
* Schlüsselwort `Section`

* Recherche mittels `apt-cache` für das Paket 'bash' -- egal ob 
...
* Recherche mittels `apt show` für das Paket 'bash' -- egal ob
  installiert, oder nicht:

----
$ apt show cleo | sed -ne '/^Section: /{s/Section: //p;}'
