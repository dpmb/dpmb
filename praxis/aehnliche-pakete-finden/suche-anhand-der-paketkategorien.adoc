...
===== Schritt 1: Ermittlung der Paketkategorie, in der das Paket einsortiert wurde =====

Dazu verfügt jedes Paket in der Paketbeschreibung über das Schlüsselwort 
`Section`. Zum Auslesen der Daten sind hier die beiden Werkzeuge 
`apt-cache` und `apt` in Kombination mit `sed` von Nutzen. Unerheblich ist, 
ob das Paket bereits installiert ist, oder nicht. Nach dem Paket 'bash' 
suchen Sie mittels `apt-cache` und `sed` wie folgt (*Variante 1*):
...
===== Schritt 2: weitere Pakete aus der Kategorie ermitteln ===== 

Hier zeigen wir das Vorgehen am Beispiel der Kategorie 'shell'. Zur 
Umsetzung stehen wieder zwei Varianten bereit -- Variante 1 nutzt `apt` 
und Variante 2 `aptitude`. Das Ergebnis ist jeweils identisch.
...
image::praxis/aehnliche-pakete-finden/gnome-packagekit-netzwerk.png[id="fig.gnome-packagekit-netzwerk", width="50%"]

==== Über einen Webbrowser ====

Die in <<webbasierte-programme>> genannten Werkzeuge zur Paketverwaltung 
erlauben keine Recherche anhand der hinterlegten Paketkategorien, was wir 
als betrüblich ansehen. Somit bleibt nur der Rückgriff auf andere 
Ressourcen. Folgende drei Quellen empfehlen wir Ihnen:

Paketsuche über die Kategorien-Übersicht auf der Debian-Webseite :: jeweils
  separiert für stabile Pakete und zukünftige Debian-Veröffentlichungen. Für
  ersteres ist das die Übersicht zu den stabilen Paketen <<Debian-Paketliste>>.
  <<fig.debian-webseite-pakete-datenbanken>> zeigt die Auflistung der Pakete 
  für die Paketkategorie 'Datenbanken' im Webbrowser Firefox.
  
.Recherche in der Kategorie Datenbanken über den Webbrowser
image::praxis/aehnliche-pakete-finden/debian-webseite-pakete-datenbanken.png[id="fig.debian-webseite-pakete-datenbanken", width="50%"]

Suche über die Paketübersicht auf der Debian-Webseite :: Einblendung von 
  ähnlichen Paketen zu einem bereits ausgewählten Paket in der rechten 
  Randleiste, ausgehend von https://packages.debian.org , hier für das 
  Paket 'iftop' (siehe grüne Einrahmung in <<fig.debian-webseite-pakete-iftop>>)

.Recherche nach ähnlichen Paketen über die Detailansicht zu dem Paket iftop
image::praxis/aehnliche-pakete-finden/debian-webseite-pakete-iftop.png[id="fig.debian-webseite-pakete-iftop", width="50%"]

Softwareübersicht im Wiki der Ubuntuusers :: Zusammenstellung im Wiki <<Ubuntu-Wiki-Softwareuebersicht>>. Es listet alle Pakete auf, die zu dieser Paketkategorie gehören

.Recherche nach ähnlichen Paketen über die Detailansicht im Wiki der Ubuntuusers
image::praxis/aehnliche-pakete-finden/ubuntuusers-kategorien.png[id="fig.ubuntuusers-kategorien", width="50%"]
