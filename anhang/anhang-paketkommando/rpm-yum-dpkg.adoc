...
angelegt werden. Für `dpkg` gehen wir darauf ``Konfigurationsdateien
eines Pakets anzeigen'' <<konfigurationsdateien-eines-pakets-anzeigen>>
genauer ein.
...
|            | rpm -ql | grep "/etc" | listet alle Dateien auf, die sich im
...
|            | rpm -ql | grep "/usr/share/doc" | listet alle Dateien auf, die sich im
