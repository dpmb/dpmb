...
|====
| Werkzeug   | Aufruf | Anmerkungen
| APT        | `apt-get upgrade` 'Paketname' | aktualisierte Version des Pakets einspielen
...
|====
| Werkzeug   | Aufruf | Anmerkungen
|  APT       | `apt-get remove` 'Paketname' | entfernt das Paket, die Konfigurationsdateien des Pakets bleiben erhalten
...
| `apt-rdepends` | `apt-rdepends` 'Paketname' | Abhängigkeiten anzeigen
|            | `apt-rdepends -r` 'Paketname' | umgekehrte Abhängigkeiten anzeigen
...
|     `      | `rpm --requires` 'Paketname' | das Paket muß dazu lokal als Datei vorliegen
...
|====
| Werkzeug   | Aufruf | Anmerkungen
| APT        | `apt-get upgrade -u` | alle Pakete auflisten, für die eine neue Version bereitsteht
...
| `RPM`      | |
| `YUM`      | `yum list updates`| alle aktualisierbaren Pakete anzeigen
|====
...
| DNF        | `dnf list available` | alle verfügbaren Pakete anzeigen
| `RPM`      | |
