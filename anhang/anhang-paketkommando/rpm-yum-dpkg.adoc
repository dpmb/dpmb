...
(((Werkzeuge zur Paketverwaltung,Vergleich mit FPM)))
(((Werkzeuge zur Paketverwaltung,Vergleich mit Guix)))
...
gegenüber, soweit dieser Schritt möglich ist. Im Rampenlicht stehen Red 
Hat Package Management (RPM), Yellowdog Updater Modified (YUM), seine 
Nachfolger Effing Package Management (FPM) und Dandified YUM (DNF), 
Debian's `dpkg`, APT sowie auch `aptitude`. Nicht fehlen darf Guix, der 
Paketmanager des GNU Guix Systems <<GNU-Guix-System>>. Wir betrachten 
dabei bspw. die Installation und die Entfernung von Paketen, das 
Auflisten der installierten und verfügbaren Pakete sowie das Anzeigen 
von Paketabhängigkeiten.

Diese Zusammenstellung basiert auf umfangreichen Tests und Recherchen,
beispielsweise mit Fedora 23, CentOS 6 und 7, Ubuntu 15.10 _Wily Werewolf_ 
sowie Debian 8 _Jessie_ bis einschließlich Debian 13 _Trixie_ und GNU Guix
System 1.5. Die Verfügbarkeit der genannten Werkzeuge und deren 
...
ein. RPM, YUM, FPM und DNF verfügen zwar über einen lokalen Paketcache, 
der jedoch zuvor manuell zu aktivieren ist. Es bestehen keine äquivalenten 
Kommandoaufrufe zu `apt`, `apt-get` und Aptitude, die diesen Cache auf den
...
(((yum, localinstall)))
(((yum, reinstall)))
Mit den nachfolgenden Aufrufen installieren Sie ein Softwarepaket über die 
Kommandozeile. Unter ``Pakete installieren'' in <<pakete-installieren>> 
gehen wir detaillierter auf `dpkg`, APT und `aptitude` ein. Unter 
``Installation zwischengespeicherter Pakete aus dem Paketcache'' in
<<installation-zwischengespeicherter-pakete-aus-dem-paketcache>> erfahren 
Sie mehr darüber, wie Sie Pakete installieren, die bereits im Paketcache 
vorliegen.
...
| Guix       | `guix install` 'Paketname' | Paket 'Paketname' wird dem Paketrepository entnommen
|            | `guix install -p` 'Profilname Paketname' | Paket 'Paketname' wird dem Paketrepository entnommen und für das angegebene Profil 'Profilname' (lokal) installiert
...
(((yum, update-to)))
(((yum, upgrade)))
Mit den nachfolgenden Aufrufen aktualisieren Sie ein Softwarepaket über die 
Kommandozeile. Unter ``Pakete aktualisieren'' in <<pakete-aktualisieren>> 
gehen wir detaillierter auf die Vorgehensweise und die begrifflichen 
Unterschiede bei `dpkg`, APT, `apt` und `aptitude` ein.
...
|            | `dnf update` 'Paketname' | nur aktualisierte Version des Pakets einspielen (veraltet)
|            | `dnf upgrade` | alle installierten Pakete aktualisieren
|            | `dnf upgrade` 'Paketname' | nur aktualisierte Version des Pakets einspielen
| Guix       | `guix upgrade` 'Paketname' | aktualisierte Version des Pakets einspielen, sofern das Paket bereits in einer früheren Version installiert ist
...
|            | `yum update` 'Paketname' | nur aktualisierte Version des Pakets einspielen
...
Softwarepakets ein (Downgrade). Für `dpkg`, `apt`, `apt-get`, `aptitude` 
und RPM besteht kein passendes Unterkommando. Unter ``Pakete downgraden'' 
in <<pakete-downgraden>> gehen wir detaillierter auf alltagstaugliche
Vorgehensweisen dazu ein. Bei GNU Guix System besteht bislang ebenfalls 
kein Unterkommando hierzu, ein Downgrade gelingt über ein Rollback der
Paketinstallation.
...
(((yum, reinstall)))
Mit den nachfolgenden Aufrufen installieren Sie ein Softwarepaket über die 
Kommandozeile erneut. Guix verfügt bislang nicht über ein entsprechendes 
Unterkommando.
...
| DNF        | `dnf reinstall` 'Paketname' | Paket wird dem Paketrepository entnommen
...
Mit den nachfolgenden Aufrufen laden Sie ein Softwarepaket herunter, 
entweder in das aktuelle Verzeichnis oder in den Paketcache. Guix 
verfügt bislang nicht über ein solches Unterkommando.

.Ein Softwarepaket nur herunterladen
[frame="topbot",options="header",cols="2,4,4",id="tab.deb-paket-herunterladen"]
|====
| Werkzeug   | Aufruf | Anmerkungen
| APT        | `apt download` 'Paketname' | Paket wird dem Paketrepository entnommen und im Paketcache gespeichert, üblicherweise im Verzeichnis `/var/cache/apt/archives/`
| `apt-get`  | `apt-get install --download-only` 'Paketname' | Paket wird dem Paketrepository entnommen und im Paketcache gespeichert, üblicherweise im Verzeichnis `/var/cache/apt/archives/`
| `aptitude` | `aptitude download` 'Paketname' | Paket wird dem Paketrepository entnommen und im Paketcache gespeichert, üblicherweise im Verzeichnis `/var/cache/apt/archives/`
...
(((yum, erase)))
(((yum, remove)))
Mit den nachfolgenden Aufrufen entfernen Sie ein Softwarepaket über die 
Kommandozeile. Unter ``Pakete deinstallieren'' in 
...
| `dpkg`     | `dpkg -r` 'Paketname' | entfernt nur das angegebene Paket
|            | `dpkg --remove` 'Paketname' | entfernt nur das angegebene Paket
...
| Guix       | `guix remove` 'Paketname' | entfernt das angegebene Paket
...
| Guix       | `guix package -I` | alle installierten Pakete auflisten
|            | `guix package --list-installed` | alle installierten Pakete auflisten
|            | `guix package -p` 'Profilname' `-I` | alle installierten Pakete des benannten Profils auflisten
|            | `guix package -p` 'Profilname' `--list-installed` | alle installierten Pakete des benannten Profils auflisten
...
| Guix       | `guix package -I` 'Paketname' | alle installierten Pakete mit dem Paketnamen auflisten
|            | `guix package --list-installed` 'Paketname' | alle installierten Pakete mit dem Paketnamen auflisten
|            | `guix package -p` 'Profilname' `-I` 'Paketname' | alle installierten Pakete mit dem Paketnamen des benannten Profils auflisten
|            | `guix package -p` 'Profilname' `--list-installed` 'Paketname' | alle installierten Pakete mit dem Paketnamen des benannten Profils auflisten
...
// (((aptitude, search ~D)))
// (((aptitude, search ~R)))
(((dnf, repoquery -q --requires)))
(((dnf, repoquery -q --whatrequires)))
(((dpkg, -f)))
(((dpkg, --field)))
(((dpkg-deb, -f)))
(((dpkg-deb, --field)))
...
(((Paket, Auflistung der Abhängigkeiten)))
(((Paket, Auflistung der Rückwärtsabhängigkeiten)))
(((Paketabhängigkeiten, Auflistung)))
(((Paketabhängigkeiten, Auflistung der Rückwärtsabhängigkeiten)))
(((repoquery, --requires)))
...
| Guix       | `guix graph` 'Paketname' | Abhängigkeiten anzeigen
|            | `guix graph --type=reverse-package` 'Paketname' | umgekehrte Abhängigkeiten anzeigen
...
|            | `yum info` 'Paketname' |
|            | `repoquery --requires` 'Paketname' | Abhängigkeiten anzeigen
|            | `repoquery -q --whatrequires` 'Paketname' | umgekehrte Abhängigkeiten anzeigen
|            | `yumdownloader --resolve` 'Paketname' |
| DNF        | `dnf repoquery -q --requires` 'Paketname' | Abhängigkeiten anzeigen
|            | `dnf repoquery -q --whatrequires` 'Paketname' umgekehrte Abhängigkeiten anzeigen
|====
...
| Guix       | `guix search` 'Muster' | alle verfügbaren Pakete anzeigen, die dieses Muster enthalten
...
| RPM        | `rpm -K` 'Paketname' | GnuPG-Signatur des Pakets prüfen
|            | `rpm --checksig` 'Paketname' | GnuPG-Signatur des Pakets prüfen
|====
...
|            | `rpm --verify --all` | prüft alle installierten Pakete
|            | `rpm --verify --package` 'Paketname' | prüft nur das angegebene Paket
| YUM        | `yum verify` 'Paketname' | Paketname prüfen; erfordert YUM-Plugin 'yum-verify'
|====
