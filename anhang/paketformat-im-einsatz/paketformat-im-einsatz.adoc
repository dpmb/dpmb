...
<<Debian-Wiki-Debian-GNUHurd>>. Andererseits gibt es außerhalb des
...
<<Watson-App-Design>>). Möglicherweise wird dieses Format vor dessen
größerer Verbreitung bereits durch den für den Herbst 2015 angekündigten
Nachfolger Snappy <<Ubuntu-Snappy>> überholt.
