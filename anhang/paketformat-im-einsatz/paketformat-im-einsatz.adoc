...
Armbian :: Debian GNU/Linux für ARM-basierte Geräte, bspw. ein Cubieboard 
oder Cubietruck <<Armbian>>.

DebWRT :: Debian GNU/Linux auf der Basis von OpenWRT für WLAN-Router. 
Eingestellt im Jahr 2010.

Raspbian :: Debian GNU/Linux so rekompiliert, dass es mit bestmöglicher 
Optimierung auch auf den schwächsten Raspberry Pis läuft <<Raspbian>>.

Raspberry Pi OS :: Eine auf Raspbian aufsetzende Distribution 
<<RaspberryPiOS>> der Raspberry Pi Foundation <<RaspberryPi>>, die auch 
nicht-freie, ``empfohlene'' Closed-Source-Software und Paketquellen von
Drittherstellern in der Standard-Installation enthält.
...
Edubuntu :: Ubuntu Linux für Schule und Bildung <<Edubuntu>>

Escuelas LINUX :: speziell auf den Bildungsbereich angepasstes
Bodhi Linux (Basis: Ubuntu Linux) <<EscuelasLinux>>

LernStick :: Live-Distribution auf einem USB-Stick <<LernStick>>

Li-f-e (Linux for Education) :: basierend auf Ubuntu Mate 22.04 und 
entwickelt für Lehrer und Schüler an Schulen und Hochschulen 
<<LinuxForEducation>>

Linkat :: Eine GNU/Linux-Distribution für den Bildungsbereich, die 
aus der Autonomen Gemeinschaft Katalonien in Spanien stammt. Das 
Betriebssystem wurde entwickelt, um die Verwendung von freier und 
quelloffener Software in Bildungseinrichtungen der Region zu fördern. 
Linkat <<Linkat>> basiert auf Ubuntu Linux.

Linux Schools :: ähnlich zu Skolelinux und mit vielen Werkzeugen für das 
Netzwerk und die Administration, jedoch statt Debian GNU/Linux auf der 
Basis von Ubuntu LTS <<LinuxSchools>>

Skolelinux :: früher DebianEdu, gedacht und konzipiert für den Einsatz in 
Schulen <<Skolelinux>>
...
vornehmlich aus dem Projekt One Laptop Per Child (OLPC) <<OLPC>> bekannt 
ist
