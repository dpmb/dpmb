...
Um die Recherche nach Softwarepaketen weiter zu vereinfachen, bestehen 
mittlerweile auch eine Reihe von Paketsuchmaschinen. Diese aggregieren diverse 
Paketquellen und machen die darüber bereitstehenden Pakete über eine 
einheitliche Schnittstelle zugänglich, zumeist eine Webseite. Bekannt sind uns
Pkgs <<pkgs>>, Repology <<repology>> und Rpmfind <<rpmfind>>. Während die 
beiden erstgenannten Projekte viele Paketformate abdecken, fokussiert Rpmfind
lediglich auf `rpm`-Pakete. In <<fig.pkgs>> sind die Veröffentlichungen und
Architekturen zu sehen, die via Pkgs verfügbar sind.

.Auswahl der Pakete für Debian GNU/Linux mittels Pkgs
image::werkzeuge/paketquellen-und-werkzeuge/pkgs.png[id="fig.pkgs", width="50%"]

Alle o.g. Paketsuchmaschinen erlauben einen detaillierten Blick auf die
einzelnen Pakete sowie deren Download. Nachteilig ist, dass der Bezug ohne Prüfung 
auf Paketabhängigkeiten stattfindet, sprich: diese mühselige Arbeit bleibt bei 
Ihnen liegen.
