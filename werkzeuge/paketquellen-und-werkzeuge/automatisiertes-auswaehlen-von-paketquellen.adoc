...
Raphael Geissert hat unter `http://http.debian.net/` den Debian
Redirector <<Debian-Redirector>> entwickelt, der eine Alternative zu
`cdn.debian.net` darstellt. Tragen Sie als Paketquelle
`http://http.debian.net/debian/` ein, sendet APT zuerst eine
HTTP-Anfrage an `http.debian.net` und bekommt als Antwort eine
...
`http.debian.net` aufgelöst wird und eine Weiterleitung (Redirect) zum
Debian-Paketmirror an der ETH Zürich erfolgt.

.Auswertung der Netzwerkkommunikation bei der Auflösung der IP
----
$ GET -SUsed http://http.debian.net/debian/
GET http://http.debian.net/debian/
...
der Hostname `http.debian.net` ab. Andererseits werden pro Paketliste
