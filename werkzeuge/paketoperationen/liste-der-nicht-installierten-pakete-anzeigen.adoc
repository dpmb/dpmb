...
auf das Paket 'bash' ein. Ergebnis ist ein einziges Paket.
...
die die Zeichenkette `ash` im Namen beinhalten, auf die keine weiteren 
Zeichen folgen. Die Angabe eines Sterns (`*`) vor bzw. nach der Zeichenkette 
interpretiert `apt` als alle Zeichen in beliebiger Menge. Daher umfaßt die
Ausgabe bspw. sowohl die Pakete 'ash' und 'bash', als auch 'crash' und 
'csmash'.
