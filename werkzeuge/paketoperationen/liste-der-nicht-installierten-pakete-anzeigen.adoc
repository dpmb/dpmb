...
Möglichkeiten zur Lösung sind uns bekannt:
...
Ergebnis ist eine alphabetisch sortierte Liste der Paketnamen ohne weitere
Angaben. *Fall b)* nutzt einen kompakteren Aufruf und kombiniert hierbei 
die Werkzeuge `dpkg`, `egrep`, `awk` und `sort` miteinander:
