...
Für die Kommandozeile existieren vier grundlegende Möglichkeiten. Ins 
Spiel kommen `dpkg`, `dpkg-query`, `aptitude` und `apt`. Die Darstellung 
in den graphischen Programme beleuchten wir weiter unten genauer.
...
welches u.a. Erweiterungen für die Bash zutagefördern:
...
Zur Recherche im Paketbestand kannt das Werkzeug `aptitude` das 
...
Beschreibung. Geht es Ihnen jedoch nur um die Namen der Pakete, hilft die
folgende Kombination aus `aptitude`, `grep`, `sed` und `awk` weiter und 
minimiert die Ausgabe erheblich:
...
`aptitude` bereitgestellt hatte, entfallen.
...
aus der Ausgabe heraus, die die gesuchten Pakete beinhalten.
...
Für den Umgang mit Mustern folgt `apt` dem gleichen Prinzip wie `dpkg`. 
Das nachfolgende Beispiel zeigt die Recherche nach allen installierten 
Paketen, die die Zeichenkette `ash` im Namen beinhalten. Die Angabe eines 
Sterns (`*`) vor und nach der Zeichenkette interpretiert `apt` als alle 
Zeichen in beliebiger Menge. 
...
Werkzeuge `awk`, `fgrep`, `awk` und `uniq` weiter. `awk` filtert den 
Paketnamen aus jeder Zeile heraus. 

.`apt` listet nur die nicht installierten Pakete auf (Paketliste)
----
$ apt list '*ash' | fgrep -v '[installiert]' | awk -F "/" ' { print $1 } ' | uniq
...
verschiedenfarbige Kästchen (``Icons''). In <<fig.smartpm-icons2>> sehen 
Sie alle Möglichkeiten in der vollständigen Übersicht. Installierte Pakete 
erkennen Sie an der grünen Farbe, weiß/hellgrau kennzeichnet nicht 
installierte Pakete und rot steht hier für defekte Pakete (Status ``broken'').
