...
(((apt-get, autoremove)))
(((apt, autoremove)))
Um den Vorgang der Paketentfernung explizit anzustoßen, verfügen `apt`
und `apt-get`` über das Unterkommando `autoremove`. Seitdem nicht nur
Aptitude sondern auch APT ein solche Funktionalität besitzt, hat die
Bedeutung von `deborphan` und `debfoster` deutlich abgenommen.
