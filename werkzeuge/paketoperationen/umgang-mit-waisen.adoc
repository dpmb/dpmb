...
entfernen. Seitdem diese Werkzeuge eine solche Funktionalität besitzen, 
hat die Bedeutung dieser spezialisierten Programme deutlich abgenommen.
...
Gtkorphan (siehe <<Gtkorphan>>). Darüber hinaus bietet `wajig` (<<wajig>>) 
entsprechende Möglichkeiten zur Suche, die wir Ihnen ebenfalls nicht 
...
Zwischen der Standardkonfiguration von APT und `aptitude` gibt es subtile 
Unterschiede, die sich über die Zeit herausgebildet haben und die es im 
Alltag zu beachten gilt. Kurz gefasst, beläßt APT verwaiste Pakete, 
während `aptitude` diese automatisch entfernt.

APT-Versionen aus der Prä-Lenny-Ära – d.h. vor Debian 5.0 _Lenny_ (2009) – 
nehmen auf die Erzeugung von Waisen kaum Rücksicht. Spätere 
Veröffentlichungen von APT achten deutlich stärker darauf und weisen Sie 
als Benutzer darauf hin. Ohne explizite Aufforderung entfernt APT keine 
Waisen. 

Bei `aptitude` ist das ganze Prozedere ein klein wenig anders. `aptitude` 
räumt in der Standard-Einstellung eigenständig auf. Das betrifft jedoch 
nur Pakete, die als ``automatisch installiert'' markiert sind und von 
denen wiederum kein manuell installiertes Paket abhängt (keine 'reverse 
dependencies' bestehen).

`aptitude` steuert das über die Option `aptitude::Delete-Unused`. Dazu 
schalten Sie dieses Verhalten entweder über die Benutzeroberfläche unter
menu:Optionen[Einstellungen] ein bzw. aus, oder ändern den Wert direkt in 
der Konfigurationsdatei von `aptitude` (siehe 
...
(((apt-get, auto-remove)))
(((apt, autoremove)))
Um ungenutzte Pakete mit APT explizit zu entfernen, verfügen `apt` und 
`apt-get` über das Unterkommando `autoremove`. `apt-get` kennt zusätzlich 
noch die alternative Schreibweise `auto-remove`. 
...
Zu beachten ist, dass `autoremove` lediglich die Paketwaisen entfernt, 
jedoch die dazugehörigen Konfigurationsdateien erhält. Hierfür kennt 
`apt-get` wiederum das Unterkommando `autopurge` als Abkürzung für 
`autoremove --purge`. In `apt` ist keine der beiden Optionen 
implementiert.

Bei `aptitude` heißt diese Option `--purge-unused`. Hierbei entfernt 
`aptitude` alle nicht notwendigen Pakete inklusive der dazugehörigen
Konfigurationsdateien. Diese Option aktivieren Sie über den Eintrag 
`aptitude::Purge-Unused` (siehe <<konfigurationsoptionen-von-aptitude>>).

[IMPORTANT]
.Verwendung der Optionen `autopurge`und `--purge-unused`
=====
Diese Optionen sind sehr mächtig und kombinieren eine ganze Reihe von 
