...
<<paketstatus-apt-cache-showpkg>>, ``Aktualisierbare Pakete anzeigen''
in <<aktualisierbare-pakete-anzeigen>> und ``Aus welchem Repo kommen die
Pakete'' in <<aus-welchem-repo-kommen-die-pakete>>). Dabei helfen ihnen
bspw. die Werkzeuge `apt-cache` mit dem Schalter `showpkg`, `aptitude`
mit dem Schalter `versions` sowie `apt-show-versions` weiter. Alle
Programme benötigen im Aufruf zusätzlich den Namen des gewünschten
Pakets. Die nachfolgende Ausgabe nutzt `aptitude` und betrifft das Paket
`openvpn` aus der stabilen Veröffentlichung. Um exakt das Paket
`openvpn` anzusprechen und diejenigen Pakete auszublenden, die so
ähnlich heißen, wurde der Paketname als regulärer Ausdruck formuliert.
