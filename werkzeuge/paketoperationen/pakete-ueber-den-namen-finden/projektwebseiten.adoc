...
sucht nach Binärpaketen mit 'suchbegriff' im Paketnamen, z.B. so für
...
Veröffentlichung 'release' an, z.B. so für die Paketbeschreibung und die
Abhängigkeiten des Paketes `screen` in Debian _Testing_:
----
...
unterstützt bisher keine Suite-Namen, es gibt bei Ubuntu bisher aber
auch nur genau einen Suite-Namen ('devel');
z.B. http://packages.ubuntu.com/de/xenial/mail/
