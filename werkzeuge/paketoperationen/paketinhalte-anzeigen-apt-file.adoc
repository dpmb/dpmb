...
`dpkg -c` 'deb-Datei' oder direkt über das `dpkg`-Hilfsprogramm
`dpkg-deb -c` 'deb-Datei'. Der Parameter 'deb-Datei' ist ein Paketarchiv
in Form einer lokal vorliegenden Datei. Befindet sich die Datei nicht im
aktuellen Verzeichnis, von dem aus Sie das Kommando aufrufen, ergänzen
Sie den dazugehörigen Verzeichnispfad.
