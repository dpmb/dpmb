...
Ihrer Beruhigung angemerkt, dass der Wechsel von Debian 6 _Squeeze_
auf Debian 7 _Wheezy_ recht unspektakulär verlief und vielfach
problemlos über die Bühne ging. Beim Wechsel von Debian 7 _Wheezy_ auf
Debian 8 _Jessie_ war vor allem der Wechsel von Apache 2.2 auf Apache
2.4 mit manuellen Anpassungen an der Apache-Konfiguration verbunden.
Beim Wechsel von Debian 8 _Jessie_ nach Debian 9 _Stretch_ muss darauf
geachtet werden, dass lokal installierte PHP-Anwendungen auch mit PHP
7.0 kompatibel sind.
