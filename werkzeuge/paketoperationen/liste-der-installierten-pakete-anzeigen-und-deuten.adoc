...
Mit dem nachfolgenden Aufruf erhalten Sie eine Liste aller Pakete, die auf 
ihrem System gerade installiert sind. `dpkg-awk` gibt nur die Zeilen aus, auf 
die der reguläre Ausdruck `Status: .* installed$` passt. Mit der Angabe 
`Package` weisen Sie `dpkg-awk` an, die Ausgabe des Ergebnisses nur auf das 
Feld Paketname zu beschränken.
...
lediglich die erste Spalte jeder Zeile aus, die den Paketnamen enthält. Alle 
