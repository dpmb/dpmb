...
Möglichkeiten zur Lösung -- a) `dpkg-query` mit einem speziellen Formatstring
für die Ausgabe in Kombination mit `egrep`, `awk` und `sort`, und b) die 
Kombination aus `dpkg`, `egrep`, `awk` und `sort`. `dpkg` selbst bietet von 
...
Fall b) ist kürzer und kombiniert `dpkg`, `egrep`, `awk` und `sort` miteinander:
