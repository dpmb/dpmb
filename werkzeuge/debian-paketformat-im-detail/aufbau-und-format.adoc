...
`deb-gview`, `dpkg-www` und `apt-browse` zur Verfügung. Während Sie mit
`deb-gview` nur auf die lokalen Dateien ihres Systems zugreifen können,
erlaubt `dpkg-www` sowohl lokale, als auch entfernte Dateien.
`apt-browse` nutzt hingegen seinen eigenen Datenbestand.
...
.Detailinformationen zum Paket 'debsums' 
