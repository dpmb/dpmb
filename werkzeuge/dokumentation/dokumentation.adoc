...
.Aufruf von `debmany` für das Paket `apt`
image::werkzeuge/dokumentation/debmany-apt.png[id="fig.debmany-apt", width="50%"]
...
`-z` :: Zur Darstellung des Auswahlmenüs wird Zenity benutzt. Default
ist `whiptail`, alternativ `dialog` oder `curl`.
