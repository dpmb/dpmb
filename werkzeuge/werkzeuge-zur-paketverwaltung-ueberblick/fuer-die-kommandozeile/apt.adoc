...
`aptsh` war ebenfalls lange Zeit produktiv nutzbar, ist aber mit
neueren APT-Versionen (1.2 und höher) nicht mehr kompatibel und wir
voraussichtlich in neueren Debian- und Ubuntu-Veröffentlichungen nicht
enthalten sein, da es nicht mehr weiterentwickelt wird
<<aptsh-verwaist-Bug-Report-831493>>.  Bis Debian 8 Jessie ist `aptsh`
aber durchaus noch benutzbar. Siehe auch <<aptsh>>.
