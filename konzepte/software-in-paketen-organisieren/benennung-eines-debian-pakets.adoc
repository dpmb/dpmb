...
(((Benennung einer Paketdatei)))
(((Paketname, Architekturangabe)))
(((Paketname, Benennung eines Debianpakets)))
(((Paketname, Namenserweiterung)))
(((Paketname, Namensschema)))
(((Paketname, Versionsangabe)))
(((Paketname, Versionsnummer)))
(((Paketname, Zeichensatz)))
(((Paketname, zulässige Zeichen bei der Benennung)))
...
Dateinamen der einzelnen Pakete in Berührung gekommen. Die Benennung einer 
Paketdatei folgt einem ausgeklügelten Schema <<Krafft-Debian-System144>>. 
Dieses Schema ist eine Konvention, die leider bei Paketen aus Drittquellen 
oft nicht eingehalten wird.

Der Dateiname besteht promär aus den drei Feldern _Paketname_, _Version_ 
und _Architektur_. Alle drei Felder werden durch einen Unterstrich '_' 
voneinander abgegrenzt. Der Dateiname endet mit einem Punkt und der 
Dateiendung '.deb'. Gemäß den Debian-Richtlinien <<Debian-Policy-Manual>> 
sind in o.g. Feldern nur ASCII-Zeichen zulässig. Unterstriche, Leerzeichen 
und Umlaute sind nicht gestattet. Das hilft dabei, Missverständnissen 
vorzubeugen und die Paketnamen auch mit allen Zeichensätzen anzeigen zu 
können.
...
(((Paketname, Architekturangabe)))
(((Paketname, Kennung eines Binärpakets)))
(((Paketname, Plattform)))
(((Paketname, Software)))
(((Paketname, Versionsangabe)))
(((Paketname, Versionsnummer)))
(((Paketname, verwendete Präfixe)))
(((Paketname, verwendete Suffixe)))
_Feld 1_ bezeichnet den Namen des Pakets, welches durch die Paketdatei
bereitgestellt wird. Als Beispiel dient die Paketdatei 
`iceweasel_3.5.16-12_i386.deb`. Diese ist ein Binärpaket (Dateiendung 
`.deb`) und beinhaltet den Webbrowser Iceweasel in der Version 3.5.16 
für die Architektur i386 (siehe <<debian-architekturen>>).
...
Bibliothek entwickeln. _dev_ ist die gebräuchliche englische Abkürzung für 
_development_ (auf deutsch für 'in Entwicklung'). Im Paket 'libxslt1-dev' 
befinden sich beispielsweise die Kopfdateien zur XSLT-1-Bibliothek.
...
// Stichworte für den Index
(((Paketname, Debian-Revision)))
(((Paketname, Upstream-Version)))
(((Paketname, Versionsangabe)))
(((Paketname, Versionsnummer)))
(((Paketname, Versionsverlauf)))
_Feld 2_ spiegelt eine Reihe unterschiedlicher Informationen und Zustände 
wieder, aus dem Sie den Versionsstand und -verlauf eines Pakets erkennen. 
Die Versionsangabe kann sowohl numerische Zeichen (Ziffern), als auch 
nichtnumerische Zeichen wie Punkte, Tilden und Buchstaben beinhalten.

Handelt es sich um ein _nicht-natives Debian-Paket_, besteht die 
Versionsnummer aus der Upstream-Version und der Debian-Revision. Bei dem 
Paket 'smartpm_1.4-2_all.deb' für 'smartpm' (siehe <<gui-smartpm>>) ist 
die Angabe 1.4 die Upstream-Version und die darauffolgende mit einem Minus 
`-` abgetrennte `2` steht für die zweite Debian-Revision. Hier liegt also 
das zweite Debianpaket vor, welches auf der Upstream-Version 1.4 basiert. 
Beinhaltet die Versionsnummer mehrere Bindestriche, ist immer der letzte 
Bindestrich der Trenner zwischen der Upstream-Version und der 
Debian-Revisionsnummer.

Handelt es sich hingegen um ein _natives Debian-Paket_, d.h. eine Software, 
die ausschließlich als Debian-Paket vertrieben wird, gibt es keine 
Debian-Revisionsnummer und die Versionsnummer des Pakets ist identisch mit 
der Versionsnummer der Software. Für das Paket 'dpkg_1.17.25_i386.deb' zu 
`dpkg` ist das 1.17.25.

// Stichworte für den Index
(((Paketname, Sortierung der Versionsangabe anhand des Releasestatus_)))
(((Paketname, Versionsangabe mit Epoche)))
Ändert sich bei der Aktualisierung (Upstream) die Versionsangabe so
grundlegend, dass die neuere Version eine kleinere Versionsnummer hat als 
die vorherige Version, so muss der Paketversion die Angabe einer mit einem 
Doppelpunkt abgetrennten _Epoche_ hinzugefügt werden. Ist bspw. die 
vorhergehende Versionsnummer 2013.06.06-4 (Upstream-Version 2013.06.06 in 
der Revision 4), entspricht das der Epoche 0 und ist identisch zu 
...
Releasestatus_ zu ermöglichen, sind eine bzw. mehrere aufeinanderfolgende 
Tilden `~` zulässig. Damit wird bspw. die Version `1.0~beta1` vor der 
Version `1.0` einsortiert. Diese Schreibweise kam zuerst bei Debian auf, 
wurde mittlerweile aber auch von anderen Open-Source-Projekten übernommen.

// Stichworte für den Index
(((Paketname, Anpassung für Ubuntu)))
(((Paketname, Backport)))
(((Paketname, Binary Non-Maintainer Upload (BinNMU))))
(((Paketname, BinNMU)))
(((Paketname, Non-Maintainer Upload (NMU))))
(((Paketname, NMU)))
...
123-3 auf Debian 8 _Jessie_. Die Ziffer 2 deklariert das Paket als die
zweite Backports-Revision des Pakets.
...
`ubuntu<n>` ::
kennzeichnet ein Debianpaket, welches für Ubuntu angepasst wurde. `<n>` 
bezeichnet die Ubuntu-Revisionsnummer. Die Angabe `121-3ubuntu4` steht für 
die vierte Ubuntu-Revision des Debian-Pakets mit der Versionsnummer 
`121-3`.
...
sind. Die nachfolgenden Beispiele verdeutlichen das: 

`xz-utils_5.4.1-1_amd64.deb` :: Paketierung der Werkzeuge des 
Komprimierungsprogramms 'xz' für die Plattform 'amd64'

`asterisk_1.8.13.1~dfsg-3+deb7u1_armhf.deb` :: Paketierung der 
Telefoniesoftware Asterisk für die ARM-Plattform 'armhf' mit 
Hardware-Floating-Point-Unterstützung 

`asciidoc_8.5.2-1_all.deb` :: das Paket Asciidoc ist plattformunabhängig 
einsetzbar
